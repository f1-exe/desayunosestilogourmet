<?php 
    ...
    if($_POST['tipo'] == 0){
                            ...
                            if(isset($stock) && !empty($stock)){
                                if(strlen($stock) > 0){
                                    if(is_numeric($stock)){
                                        if($categoria > 0){
                                            if(subirIMG()){
                                                if(editarProductoConImagen($idProd, $nombre, $precio, $stock, $categoria, $nameImagen, $detalle)){

                                                    echo "Se ha editado el producto correctamente";
                                                }else{
        ...
        $categoria = $_POST["categoria"];
        $detalle = $_POST["detalle"];
        if(editarProductoSinImagen($idProd, $nombre, $precio, $stock, $categoria, $detalle)){
            echo "Se ha editado el producto correctamente";
        }else{
            echo "Error al modificar en la Base de Datos";
        }
